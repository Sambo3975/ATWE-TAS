...
do
    ...
    function pauseplus.createDefaultMenu(canSave,saveSFX) -- Creates a copy of the default menu.
        ...
        if canSave then
            ...
            pauseplus.createOption("main",{text = "Save & Quit",actions = {pauseplus.save,pauseplus.quit}})
        else
            pauseplus.createOption("main",{text = "Quit",action = pauseplus.quit})
        end
    end
end
...
do
    local function getMovementDirection(back,forward,currentOption,minOption,maxOption,step)
        ...
        if player.keys[back] == KEYS_PRESSED and (currentOption-step) >= minOption then
            return -1*step
        elseif player.keys[forward] == KEYS_PRESSED and (currentOption+step) <= maxOption then
            return 1*step
        end
    end
...
pauseplus.font = textplus.loadFont("pauseplus_font.ini")
