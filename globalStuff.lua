...
globalStuff.maxCoins  = 9999
globalStuff.maxDeaths = 99999999999999


globalStuff.isOnMap = (Level.filename() == smwMap.levelFilename)
globalStuff.isInIntro = (Level.filename() == levels.names.intro)

globalStuff.messWithMapHUD = true



globalStuff.isInPlusThing = (player.character == CHARACTER_LUIGI)
globalStuff.levelPalette = (globalStuff.isInPlusThing and 1) or 0


globalStuff.exitFadeActive = false
globalStuff.exitFadeOut = 0


globalStuff.speedrunModeEnabled = false
...
globalStuff.screenMode = SCREEN_MODE_WIDE


--globalStuff.explosionID = Explosion.register(76, 953, 43, true, false)
globalStuff.explosionID = Explosion.register(64, 953, Misc.resolveSoundFile("thwomp_loud"), true, false)
...
function globalStuff.getShopItems()
...
function globalStuff.startFadeOut()
    globalStuff.exitFadeActive = true
    Misc.pause(true)
end
...
do
    ...
    globalStuff.muteCoinGainingSound = false

    function globalStuff.updateCoins()
        local forceSound = false


        -- Coin stuff
        SaveData.coins = math.min(globalStuff.maxCoins,SaveData.coins + mem(REAL_COINS_ADDR,FIELD_WORD))
        ...
        if lifeAddition > 0 then
            SaveData.coins = math.min(globalStuff.maxCoins,SaveData.coins + (coinsPerLives[lifeAddition] or coinsPerLives[#coinsPerLives]))
            forceSound = true
        end
        ...
        if SaveData.coins ~= displayCoins or forceSound then
            if not globalStuff.muteCoinGainingSound then
                SFX.play(14)
            end
        else
            globalStuff.muteCoinGainingSound = false
    ...
    function globalStuff.updateDeaths(fromRespawn)
        if fromRespawn then
            SaveData.totalDeaths = math.min(globalStuff.maxDeaths,SaveData.totalDeaths + 1)
            levelDeaths = math.min(globalStuff.maxDeaths,levelDeaths + 1)
        end
    end
end
...
function globalStuff.onTickEnd()
...
function globalStuff.replaceEffects()
...
do
    ...
    local function drawHUD(camIdx,priority,isSplit)
        ...
        if globalStuff.isUsingFullMode() then
        ...
        local putEverythingOnLeft = globalStuff.isUsingFullMode() or (globalStuff.speedrunModeEnabled and globalStuff.screenMode == SCREEN_MODE_SNES)
    ...
    function globalStuff.drawSpeedrunStuff(priority)
        if not globalStuff.speedrunModeEnabled then
            return
        end
        ...
        local flashInterval = (globalStuff.levelBeatTimer / 24)
        local flashingActive = (flashInterval > 0 and flashInterval <= 8)
        local drawTimes = (not flashingActive or flashInterval%1 < 0.5)

        local displayLevelTime = (flashingActive and globalStuff.beatLevelTime) or lunatime.drawtime()
        ...
        for _,buttonData in ipairs(displayButtons) do
            if player.keys[buttonData[1]] then
        ...
        local newTime = globalStuff.beatLevelTime
    ...
    function globalStuff.drawMusicName(priority)
    ...
    function globalStuff.updateMusicName()
...
do
    ...
    globalStuff.saveSound = SFX.open(Misc.resolveSoundFile("saved"))
    ...
    local function saveAndQuitRoutine()
        ...
        SFX.play(globalStuff.saveSound)


        Routine.wait(0.5,true)

        globalStuff.startFadeOut()
    end
    ...
    function globalStuff.initPauseMenuOptions()
        -- Main
        ...
        if not globalStuff.isOnMap then
        ...
        --pauseplus.createOption("settings",{text = "Border Style",selectionType = pauseplus.SELECTION_NAMES,selectionNames = {"Dark Symbols","Symbols","Black"},action = globalStuff.initOptionChoices,description = "Choose the the border surrounding the window."})
        pauseplus.createOption("settings",{text = "Screen Mode",selectionType = pauseplus.SELECTION_NAMES,selectionNames = {SCREEN_MODE_WIDE,SCREEN_MODE_SNES,SCREEN_MODE_FULL,SCREEN_MODE_FULL_WIDE},action = globalStuff.initOptionChoices,description = "Select how the screen fills the window."})
        pauseplus.createOption("settings",{text = "Mute Music",selectionType = pauseplus.SELECTION_CHECKBOX,action = globalStuff.initOptionChoices,description = "Mute all music from the game."})
        pauseplus.createOption("settings",{text = "Quick Death",selectionType = pauseplus.SELECTION_CHECKBOX,action = globalStuff.initOptionChoices,description = "Speeds up the death effect."})
        ...
        --pauseplus.createOption("settings",{text = "Speedrun Mode",selectionType = pauseplus.SELECTION_CHECKBOX,action = globalStuff.initOptionChoices,description = "Show a timer and input display."})
        ...
        pauseplus.createOption("speedrun",{text = "Speedrun HUD",selectionType = pauseplus.SELECTION_CHECKBOX,action = globalStuff.initOptionChoices,description = "Enable a timer and input display."})
        ...
        if not globalStuff.isOnMap and fastestTimes[Level.filename()] ~= nil then
            ...
            pauseplus.createOption("eraseLevelTime",{text = "Yes, erase this level's PB",goToSubmenu = "speedrun",action = (function(optionObj)
                ...
                globalStuff.initPauseMenuOptions()
        ...
        if stats ~= nil then
            ...
            local menuName = (globalStuff.isOnMap and "Map Stats") or "Level Stats"
    ...
    globalStuff.dontSetSectionBounds = {}

    function globalStuff.initSectionBounds()
        -- Handle section bounds
        for _,sectionObj in ipairs(Section.get()) do
            if not globalStuff.dontSetSectionBounds[sectionObj.idx] then
    ...
    function globalStuff.isUsingFullMode()
        return screenModes[globalStuff.screenMode].full
    end


    function globalStuff.initOptionChoices()
        globalStuff.screenMode = pauseplus.getSelectionValue("settings","Screen Mode")
        globalStuff.speedrunModeEnabled = pauseplus.getSelectionValue("speedrun","Speedrun HUD")

        if globalStuff.isOnMap then
            if globalStuff.messWithMapHUD then
                local screenModeProperties = screenModes[globalStuff.screenMode]
                ...
                if globalStuff.screenMode == SCREEN_MODE_WIDE then
                    smwMap.hudSettings.starcoinsXOffset = 16
                    smwMap.hudSettings.starcoinsAtBottom = false
                else
                ...
                smwMap.hudSettings.borderImage = globalStuff.getBorderImage()
        ...
        -- Wide mode
        local modeProperties = screenModes[globalStuff.screenMode]
    ...
    globalStuff.midAirJumpsUsed = 0
    globalStuff.crushingBlocks = {}
    ...
    function globalStuff.handleAssist()
        -- Restore crushing blocks
        for i = #globalStuff.crushingBlocks, 1, -1 do
            local block = globalStuff.crushingBlocks[i]
            local remove = false
            ...
            if remove then
                table.remove(globalStuff.crushingBlocks,i)
            end
        end
        ...
        if isOnGroundRedigit() or player:mem(0x26,FIELD_WORD) > 0 or player:mem(0x34,FIELD_WORD) > 0 then
            globalStuff.midAirJumpsUsed = 0
        elseif (player.forcedState == FORCEDSTATE_NONE and player.deathTimer == 0 and not player.climbing) and globalStuff.midAirJumpsUsed < allowedJumps and player.keys.jump == KEYS_PRESSED then
            globalStuff.midAirJumpsUsed = globalStuff.midAirJumpsUsed + 1
    ...
    function globalStuff.handleAssistOnDraw()
        for _,block in ipairs(globalStuff.crushingBlocks) do
    ...
    function globalStuff.onPlayerKill(eventObj,p)
        if pauseplus.getSelectionValue("assist","Invincibility") then
            ...
            if #blocks > 0 then
                for _,block in ipairs(blocks) do
                    block.isHidden = true
                    table.insert(globalStuff.crushingBlocks,block)
    ...
    function globalStuff.getBorderImage()
        if borderImages[isWide] == nil then
            local imageName = "border_".. screenModes[globalStuff.screenMode].name
            ...
            borderImages[globalStuff.screenMode] = Graphics.loadImage(Misc.resolveGraphicsFile(imageName) or Misc.resolveGraphicsFile("stock-0.png"))
        end

        return borderImages[globalStuff.screenMode]
    end

    function globalStuff.drawBorder()
        if globalStuff.isOnMap or globalStuff.isUsingFullMode() then
            return
        end

        local image = globalStuff.getBorderImage()
...
do
    ...
    function globalStuff.handleDeathEffects()
    ...
    function globalStuff.createDeathEffect(p)
        local obj = {}
        ...
        globalStuff.muteCoinGainingSound = true
    end

    function globalStuff.onResetDeathEffects(fromRespawn)
        if fromRespawn then
            deathEffects = {}
            deathCoins = {}
        end
...
do
    ...
    function globalStuff.applyLevelPalette()
        if globalStuff.levelPalette > 0 then
            local shaderObj,uniforms = paletteChange.getShaderAndUniforms(globalStuff.levelPalette,"palette.png")
...
--[[do
    ...
    local function startSaveIcon(successful)
        if Misc.inEditor() or Misc.inMarioChallenge() or globalStuff.isOnMap then
            return
        end
    ...
    function globalStuff.queueSaveIcon(successful)
    ...
    function globalStuff.onSaveGameEarly()
        SaveData.saveCount = SaveData.saveCount + 1
    end

    function globalStuff.onSaveGameLate()
        globalStuff.queueSaveIcon(saveWasSuccessful())
    end


    function globalStuff.onDrawSaveIcon()
    ...
    function globalStuff.onStartSaveIcon()
        if GameData.saveIconQueued then
            startSaveIcon(GameData.queuedSaveIconSuccessful)
            GameData.saveIconQueued = true
        end
...
local function introRoutine()
    ...
    if player.keys.dropItem and player.keys.altJump then -- had to change this so inputs can be manipulated programmaticaly
        SFX.play(14)

        SaveData.pauseplus.selectionData.speedrun["speedrun hud"] = true
        globalStuff.speedrunModeEnabled = true
...
globalStuff.beatLevelTime = 0
globalStuff.levelBeatTimer = 0


function globalStuff.onDraw()
    if globalStuff.exitFadeActive then
        globalStuff.exitFadeOut = math.min(1, globalStuff.exitFadeOut + 0.02)

        if globalStuff.exitFadeOut >= 1 then
            if globalStuff.isOnMap then
                pauseplus.quit()
            else
                Misc.unpause()
        ...
        Graphics.drawScreen{color = Color.black.. globalStuff.exitFadeOut,priority = 9}
    end


    if Level.winState() == 0 then
        globalStuff.beatLevelTime = lunatime.drawtime()
        globalStuff.levelBeatTimer = 0
    else
        globalStuff.levelBeatTimer = globalStuff.levelBeatTimer + 1
    end
    ...
    globalStuff.drawSpeedrunStuff(hudoverride.priority + 0.001)
    globalStuff.drawMusicName(hudoverride.priority - 0.001)
end


function globalStuff.onReset(fromRespawn)
    globalStuff.initSectionBounds()
end


function globalStuff.onExitLevel(winType)
...
globalStuff.horizontalShake = 0
globalStuff.verticalShake = 0

function globalStuff.onCameraUpdate()
    -- Screen shake
    globalStuff.horizontalShake = math.max(globalStuff.horizontalShake - 1,0)
    globalStuff.verticalShake = math.max(globalStuff.verticalShake - 1,(Defines.earthquake - 1)*1.5,0)

    local multiplier = ((lunatime.drawtick() % 2)*2 - 1) * screenshakeOptions[pauseplus.getSelectionValue("settings","Screenshake")]

    camera.x = camera.x + globalStuff.horizontalShake*multiplier
    camera.y = camera.y + globalStuff.verticalShake*multiplier

    Defines.earthquake = 0
end


function globalStuff.onStart()
    if globalStuff.isOnMap and not SaveData.sawIntro and not Misc.inEditor() and not Misc.inMarioChallenge() then
    ...
    elseif globalStuff.isInIntro then
...
function globalStuff.onCheckpoint(c,_)
...
function globalStuff.onPostExplosion(explosion,p)
    if explosion.id == globalStuff.explosionID then
...
function globalStuff.onInitAPI()
    registerEvent(globalStuff,"onStart","initOptionChoices",false)
    registerEvent(globalStuff,"onDraw","drawBorder",false)

    registerEvent(globalStuff,"onTick","handleAssist")
    registerEvent(globalStuff,"onDraw","handleAssistOnDraw")

    registerEvent(globalStuff,"onDraw","updateCoins")
    registerEvent(globalStuff,"onReset","updateDeaths")

    registerEvent(globalStuff,"onPostNPCHarm")
    registerEvent(globalStuff,"onPostNPCKill")

    registerEvent(globalStuff,"onTick","replaceEffects")
    registerEvent(globalStuff,"onTickEnd","replaceEffects")
    registerEvent(globalStuff,"onDraw","replaceEffects")

    registerEvent(globalStuff,"onDraw","applyLevelPalette",false)

    registerEvent(globalStuff,"onDraw","handleDeathEffects")
    registerEvent(globalStuff,"onPostPlayerKill","createDeathEffect")
    registerEvent(globalStuff,"onReset","onResetDeathEffects")


    --registerEvent(globalStuff,"onSaveGame","onSaveGameEarly",true)
    --registerEvent(globalStuff,"onSaveGame","onSaveGameLate",false)
    --registerEvent(globalStuff,"onDraw","onDrawSaveIcon")
    --registerEvent(globalStuff,"onStart","onStartSaveIcon")


    registerEvent(globalStuff,"onStart","onStart",false)

    registerEvent(globalStuff,"onTick")
    registerEvent(globalStuff,"onTickEnd")
    registerEvent(globalStuff,"onDraw")

    registerEvent(globalStuff,"onCameraUpdate")

    registerEvent(globalStuff,"onPlayerKill")

    registerEvent(globalStuff,"onReset")

    registerEvent(globalStuff,"onExitLevel")

    registerEvent(globalStuff,"onCheckpoint")

    registerEvent(globalStuff,"onPostExplosion")
end
